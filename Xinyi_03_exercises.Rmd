...
garden_harvest%>%
  ...
  geom_bar()+
  labs(title = "Tomatoes Varieties",
       x="count",y="tomato variety")+
...
garden_harvest%>%
  ...
  mutate(variety2 = fct_reorder(variety, length, .desc = TRUE)) %>% 
  group_by(variety2)%>%
  distinct(variety2,.keep_all = TRUE)%>%
  arrange(variety2)
```
...
```{r}
ggplot(Trips,aes(x=sdate))+geom_density(fill="gray")
```
...
Trips%>% 
  ...
  ggplot(aes(x=time_of_day)) + geom_density(fill="gray")
```
...
ggplot(Trips, aes(x=time_of_day)) + geom_density(aes(fill=Trips$client), alpha = 0.5, color = NA) + facet_wrap(~day)  
```{r}
Trips%>%
  mutate(time_of_day = hour(sdate) +
           minute(sdate)/60,
...
ggplot(Trips,aes(x = time_of_day)) + 
  geom_density(aes(fill=client), alpha=.4, color=NA) + facet_wrap(~ wday)
```{r}
Trips%>%
  mutate(time_of_day = hour(sdate) +
           minute(sdate)/60,
...
Trips%>%
  ...
  mutate(wday = ifelse(wday(sdate) %in% c(1,7), "weekend", "weekday"))%>%
  ggplot(aes(x=time_of_day))+
  geom_density(aes(fill=wday),alpha=0.5,color=NA)+
  facet_wrap(~client)
```
...
kids%>%
  ...
  geom_line()+
  #geom_text(aes(x =0, y = 1000,label=state),col="white")+
  facet_geo(vars(state))+
  labs(x="",y="",
       title="Change in public spending on libraires from 1997 to 2016",
